...
The syntax() static method returns the list of the markers:

    my ($start_marker, $end_marker_id, $end_marker) = Text::MagicTemplate->syntax;

See L<"Redefine Syntax"> to have more details.

=item code_execution_OFF ()

This static method disables the execution of subroutines from your code. (see L<"CAVEATS"> for details).

=item code_execution_ON ()

This static method enables the execution of subroutines from your code. Since this feature is enabled by default, you need it only if you have used the code_execution_OFF() static method before.
...
Deprecated method maintained for backward compatibility: use code_execution_OFF() instead.

=item code_execution ()

Deprecated method maintained for backward compatibility: use code_execution_ON() instead.
...
The action generated by Text::MagicTemplate and triggered by the value type. (i.e. an UNDEF value type trigger the deletion of the label or the block, a ARRAY value type generate a loop).
