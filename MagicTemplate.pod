...
The Text::MagicTemplate object is a blessed hash containing locations that it uses to look up I<identifiers>. The locations array can store package namespaces or references to hashes. If no parameter is passed to the constructor, the package namespace of the caller will be used by default.
...
B<The VALUE TYPE of the found value will determine the behaviour of MagicTemplate>:

=over

=item *

An B<UNDEF> value type will B<delete> I<label> or I<block>.

=item *

A B<SCALAR> value type will B<replace> I<label> or I<block> with the scalar value.

=item *

A B<HASH> value type will set that HASH as a B<temporary location> for the lookup of the I<block>. Text::MagicTemplate first uses that location to look up the identifiers contained in the block; then, if unsuccessful, it will search into the stored locations.

=item *

An B<ARRAY> value type will B<generate a loop>, merging each value in the array with the the I<block> and replacing the block with the sequence of the outputs.
...
If you want to keep unavailable some variable or subroutine from the template, you can pass just the reference of some hash containing just the identifiers used in the template. This is the best method to use the module IF you allow untrustworthy people to edit the template AND if you have any potentially dangerous subroutine in your code. (see L<"CAVEATS">).
...
NOTE: Each parameter of syntax static method is parsed as a regular expression as: C<qr/parameter/>, so you can extend the syntax beyond a static string marker. This syntax:

    Text::MagicTemplate->syntax ('\d{3}', '\W', '\d{3}'); # weird markers :-)
...
This static method enable or disables the execution of subroutines from your code. Default enabled. (see L<"CAVEATS"> for details).

    Text::MagicTemplate->subs_execution( 0 );
    Text::MagicTemplate->subs_execution( 1 );

=item get_block ( template [, identifier] )

This static method returns a reference to the template content or to a block inside the template, without merging values. It accepts one I<template> parameter that can be a reference to a SCALAR content, a path to a template file or a filehandle. If any I<identifier> is passed, it returns just that block.
...
=item Deprecated methods

C<no_code_execution()>, C<code_execution()>, C<code_execution_ON()>, C<code_execution_OFF()> : use subs_execution() instead.
...
This is an example that explains you how to write and use a custom syntax module F<Local::myCustomSyntax.pm> I<(obviously you can use the class name you prefer)>.

Redefine the markers and save this code as file F<Local::myCustomSyntax.pm>

    package Local::myCustomSyntax; # choose a meaningful package namespace :-)
    use Text::MagicTemplate;
...
Note that if you use a subclass - as C<Text::MagicTemplate::HTML> - you must use the correct subclass name:
    ...
    Text::MagicTemplate::HTML->subs_execution(0);
    # or
    Local::myCustomSyntax->subs_execution(0);
...
The action generated by Text::MagicTemplate and triggered by the value type. (i.e. an UNDEF value type triggers the deletion of the label or the block, a ARRAY value type generate a loop).
...
The type of the value found by a lookup (i.e. UNDEF, SCALAR, HASH, ARRAY), that trigger a specific behaviour.
